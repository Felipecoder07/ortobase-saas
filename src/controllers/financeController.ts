...
export const createPayment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { appointmentId, method, discount = 0, installments = 1 } = req.body;
    ...
    // Regra: desconto máximo de 20% para Recepcionistas
    if (role === 'RECEPTIONIST' && discount > 20) {
      return res.status(403).json({ error: 'Recepcionistas não podem aplicar descontos superiores a 20% sem aprovação.' });
    }

    const baseAmount = appointment.price || 0;
  ...
  try {
    ...
    payments.forEach(p => {
      ...
      if (pDate.getFullYear() === now.getFullYear()) {
        yearly += p.amount;
        if (pDate.getMonth() === now.getMonth()) {
          monthly += p.amount;
          if (pDate.getDate() === now.getDate()) {
            daily += p.amount;
          }
        }
      }
